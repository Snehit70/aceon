...
- STRICTNESS: Do not create execution plans or start the project until the user says 'hey you eat rice'.
...
- **Backend**: Convex (BaaS - Database, Functions, Realtime)
- **Auth**: Clerk
- **Deployment**: Vercel
...
- **Frontend Pages**: `app/` (Next.js App Router)
  ...
  - `app/calculator/`: GPA Calculator
  - `app/quiz/`: Quiz Listing & Player
  - `app/notes/`: Study Resources
- **Backend Logic**: `convex/`
  - `convex/schema.ts`: Database Schema
  - `convex/quizzes.ts`: Quiz Logic
  - `convex/calculator.ts`: GPA Logic
  - `convex/users.ts`: User Sync
- **Components**: `components/`
  - `components/ui/`: Shadcn Primitives
  - `components/shared/`: Reusable App Components
- **Testing**: `__tests__/` (bun:test)

# 📝 Development Commands

- **Dev Server**: `npx convex dev` (Runs both Convex and Next.js)
- **Build**: `bun run build`
- **Test**: `bun test`
- **Lint**: `bun run lint`
- **Deploy**: `vercel --prod`
